...
# Show columns with text/kind 'text' that might be descriptions
print('=== PJ2 columns with text descriptions ===')
for c in sorted(cols, key=lambda s: (len(s), s)):
    items = [(r, k, s) for r, k, s in cols[c]]
    text_items = [(r, s) for r, k, s in items if k == 'text']
    if text_items:
        # Show first few
        print(f'Column {c}: {len(text_items)} text cells')
        for r, s in text_items[:3]:
            print(f'  Row {r}: {s[:50]}')
        print()